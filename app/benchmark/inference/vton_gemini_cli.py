...
PROMPT = """
Perform virtual try-on task for this person image and garment. Replace the aodai of person with provided one.
...
def parseArgs():
    parser = argparse.ArgumentParser(description="Perform virtual try-on using Gemini.")
...
def main():
    ...
    for sample in tqdm(dataset, "Processing"):
        ...
        person_path = sample["human_image"]
        cloth_path = sample["vton_garment_path"]

        try:
            person_image = Image.open(person_path).resize((width, height))
            garment_image = Image.open(cloth_path)
        except FileNotFoundError as e:
            print(f"\nError: File not found: {e}")
            continue
        except Exception as e:
            print(f"\nAn error occurred while opening the images: {e}")
            continue
        ...
        try:
            response = model.generate_content(
                [formatted_prompt, person_image, garment_image],
