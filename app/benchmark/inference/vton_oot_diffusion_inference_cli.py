...
from source.benchmark.vton.OOTDiffusionVirtualTryOn import OOTDiffusionVirtualTryOn
...
def parseArgs():
    parser = argparse.ArgumentParser(description="Perform virtual try-on using OOTDiffusionVirtualTryOn.")
...
if __name__ == "__main__":
    ...
    model_root = Path(args.pretrained_dir) / "oot_diffusion"
    
    oot_diffusion_vton = OOTDiffusionVirtualTryOn(
        model_root=str(model_root),
        offload=args.offload,
        aggressive_offload=args.aggressive_offload
    )
    
    oot_diffusion_vton.process(
        dataset,
        args.output_dir,
