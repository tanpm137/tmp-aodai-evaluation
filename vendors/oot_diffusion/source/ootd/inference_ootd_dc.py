...
class OOTDiffusionDC:
    ...
    def __init__(self, gpu_id, checkpoints_path="checkpoints"):
        self.gpu_id = 'cuda:' + str(gpu_id)
        
        VIT_PATH = os.path.join(checkpoints_path, "clip-vit-large-patch14")
        VAE_PATH = os.path.join(checkpoints_path, "ootd")
        UNET_PATH = os.path.join(checkpoints_path, "ootd/ootd_dc")
        MODEL_PATH = os.path.join(checkpoints_path, "ootd")

        vae = AutoencoderKL.from_pretrained(
            VAE_PATH,
            subfolder="vae",
            torch_dtype=torch.float16,
            use_safetensores=False
        )
