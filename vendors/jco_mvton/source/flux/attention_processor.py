...
class Attention(nn.Module):
    ...
    def __init__(
        ...
        if qk_norm is None:
            ...
            if mode == 1:
                self.extra_norm_q, self.extra_norm_k = nn.ModuleList([]),nn.ModuleList([])
                for i in range(self.extra_branch_num):
                    self.extra_norm_q.append(RMSNorm(dim_head, eps=eps))
                    self.extra_norm_k.append(RMSNorm(dim_head, eps=eps))
        elif qk_norm == "l2":
        ...
        self.extra_branch_num = extra_branch_num
        if self.extra_branch_num > 0:
            self.extra_to_q = nn.ModuleList([nn.Linear(query_dim, self.inner_dim, bias=bias) for i in range(extra_branch_num)])
            if not self.only_cross_attention:
                self.extra_to_k = nn.ModuleList([nn.Linear(self.cross_attention_dim, self.inner_dim, bias=bias) for i in range(extra_branch_num)])
                self.extra_to_v = nn.ModuleList([nn.Linear(self.cross_attention_dim, self.inner_dim, bias=bias) for i in range(extra_branch_num)])
            else:
                self.extra_to_k = None
                self.extra_to_v = None
        if not self.pre_only:
            self.to_out = nn.ModuleList([])
            self.to_out.append(nn.Linear(self.inner_dim, self.out_dim, bias=out_bias))
            self.to_out.append(nn.Dropout(dropout))
            if self.extra_branch_num > 0 and mode == 1:
                self.extra_to_out = nn.ModuleList([])
                self.extra_to_out.append(nn.ModuleList(nn.Linear(self.inner_dim, self.out_dim, bias=out_bias) for i in range(extra_branch_num)))
                self.extra_to_out.append(nn.ModuleList(nn.Dropout(dropout)for i in range(extra_branch_num)))
...
class FluxAttnProcessor2_0:
    ...
    def __call__(
        ...
        attention_mask: Optional[torch.FloatTensor] = None,
        image_rotary_emb: Optional[torch.Tensor] = None,
        extra_norm_hidden_states:torch.FloatTensor = None,
        mode = 1,
        cond_true=[1,1],
    ) -> torch.FloatTensor:
        batch_size, _, _ = hidden_states.shape if encoder_hidden_states is None else encoder_hidden_states.shape
        # `sample` projections.
        length = attn.extra_branch_num
        dim1 = hidden_states.shape[1]    
        if encoder_hidden_states is not None:
            dim2 = encoder_hidden_states.shape[1]
        else:
            dim2 = 0
        if extra_norm_hidden_states is not None:
            dims = []
            for i in range(len(extra_norm_hidden_states)):
                dims.append(extra_norm_hidden_states[i].shape[1])
        ...
        if extra_norm_hidden_states is not None:
            ...
            for i in range(length):
                if cond_true[i] == 0:
                    continue
                extra_hidden_states_query_proj = attn.extra_to_q[i](extra_norm_hidden_states[j])
                extra_hidden_states_key_proj = attn.extra_to_k[i](extra_norm_hidden_states[j])
                extra_hidden_states_value_proj = attn.extra_to_v[i](extra_norm_hidden_states[j])
                j += 1
                extra_hidden_states_query_proj = extra_hidden_states_query_proj.view(
                    batch_size, -1, attn.heads, head_dim
                ).transpose(1, 2)
                extra_hidden_states_key_proj = extra_hidden_states_key_proj.view(
                    batch_size, -1, attn.heads, head_dim
                ).transpose(1, 2)
                extra_hidden_states_value_proj = extra_hidden_states_value_proj.view(
                    batch_size, -1, attn.heads, head_dim
                ).transpose(1, 2)
                if mode == 1:
                    if attn.extra_norm_q[i] is not None:
                        extra_hidden_states_query_proj = attn.extra_norm_q[i](extra_hidden_states_query_proj)
                    if attn.extra_norm_k[i] is not None:
                        extra_hidden_states_key_proj = attn.extra_norm_k[i](extra_hidden_states_key_proj)
                elif mode == 2:
                    if attn.norm_q is not None:
                        extra_hidden_states_query_proj = attn.norm_q(extra_hidden_states_query_proj)
                    if attn.norm_k is not None:
                        extra_hidden_states_key_proj = attn.norm_k(extra_hidden_states_key_proj)
                query = torch.cat([query, extra_hidden_states_query_proj], dim=2)
                key = torch.cat([key, extra_hidden_states_key_proj], dim=2)
                value = torch.cat([value, extra_hidden_states_value_proj], dim=2)
        ...
        if image_rotary_emb is not None:
            ...
            query = apply_rotary_emb(query, image_rotary_emb).to(value.dtype)
            key = apply_rotary_emb(key, image_rotary_emb).to(value.dtype)
        q1,k1,v1 = query[:,:,:dim1 + dim2,:],key[:,:,:dim1 + dim2,:],value[:,:,:dim1 + dim2,:]
        qs,ks,vs=[],[],[]
        p = dim1 + dim2
        for i in range(len(extra_norm_hidden_states)):
            l = p; r = p + dims[i]; p = r
            q,k,v =  query[:,:,l:r,:],key[:,:,l:r,:],value[:,:,l:r,:]
            qs.append(q);ks.append(k);vs.append(v)
        hs = []
        h1 = F.scaled_dot_product_attention(q1, key, value, dropout_p=0.0, is_causal=False)
        for i in range(len(extra_norm_hidden_states)):
            q = qs[i]
            k = ks[i]
            v = vs[i]
            cond_hs = F.scaled_dot_product_attention(q, k, v, dropout_p=0.0, is_causal=False)
            hs.append(cond_hs)
        hidden_states = h1
        for i in range(len(extra_norm_hidden_states)):   
            hidden_states = torch.cat((hidden_states,hs[i]),dim=2)
        hidden_states = hidden_states.transpose(1, 2).reshape(batch_size, -1, attn.heads * head_dim)
        if extra_norm_hidden_states is not None:
            extra_hidden_states = []
            p = dim1+ dim2
            for i in range(len(extra_norm_hidden_states)):
                extra_hidden_states.append(hidden_states[:, p: p + dims[i]])
                p = p + dims[i]
        if encoder_hidden_states is not None:
            j = 0
            for i in range(length):
                if cond_true[i] == 0:
                    continue
                if mode == 1:
                    _extra_hidden_states = attn.extra_to_out[0][i](extra_hidden_states[j])
                    extra_hidden_states[j] = attn.extra_to_out[1][i](_extra_hidden_states)
                elif mode == 2:
                    _extra_hidden_states = attn.to_out[0](extra_hidden_states[j])
                    extra_hidden_states[j] = attn.to_out[1](_extra_hidden_states)
                j = j + 1
            encoder_hidden_states, hidden_states = (
                hidden_states[:, : dim2],
                hidden_states[:, dim2 : dim1 + dim2],
            )
            # linear proj
            hidden_states = attn.to_out[0](hidden_states)
            # dropout
            ...
            encoder_hidden_states = attn.to_add_out(encoder_hidden_states)
            return hidden_states, encoder_hidden_states, extra_hidden_states
        else:
            return hidden_states
