...
class Attention(nn.Module):
    ...
    def forward(
        ...
        **cross_attention_kwargs,
        ...
        quiet_attn_parameters = {"ip_adapter_masks",'garm_ref'}
        unused_kwargs = [
...
class AttnProcessor2_0:
    ...
    def __call__(
        ...
        attention_mask: Optional[torch.Tensor] = None,
        temb: Optional[torch.Tensor] = None,
        garm_ref: Optional[torch.Tensor] = None,
        *args,
        **kwargs,
    ) -> torch.Tensor:
        if len(args) > 0 or kwargs.get("scale", None) is not None:
        ...
        if garm_ref is not None:
            # print("garm_ref")
            encoder_hidden_states = torch.cat([encoder_hidden_states, garm_ref], dim=1)
